...
class Game {
  ...
  constructor( socket) {
    ...
    this.lanes = [this.LEFT_HAND, this.RIHGT_HAND, this.LEFT_FOOT, this.RIGHT_FOOT];
    this.targets = [
    {x: paper.view.center.x - 177, y: this.metrumY + 34},
    {x: paper.view.center.x + 177, y: this.metrumY + 34},
    {x: paper.view.center.x - 92, y: this.metrumY + 208},
    {x: paper.view.center.x + 92, y: this.metrumY + 208}];
    this.starts = [{x: -40, y: 400}, {x: 1064, y: 400}, {x: -40, y: this.metrumY + 208}, {x: 1064, y: this.metrumY + 208}];
  ...
  socketSetup() {
    this.socket.on('leftHandDown', () => {
      ...
      createjs.Tween.get(this.leftArm, {loop: false, override: true}).to({rotation: 40}, 250).to({rotation: 0}, 400);

      this.checkHandHit(this.LEFT_HAND);
    });

    this.socket.on('rightHandDown', () => {
      createjs.Tween.get(this.rightArm, {loop: false, override: true}).to({rotation: -40}, 250).to({rotation: 0}, 400);
      this.checkHandHit(this.RIHGT_HAND);
    });

    this.socket.on('leftFootDown', () => {
      createjs.Tween.get(this.leftLeg, {loop: false, override: true}).to({rotation: 20}, 250).to({rotation: 0}, 400);
      this.checkHandHit(this.LEFT_FOOT);
    });

    this.socket.on('rightFootDown', () => {
      createjs.Tween.get(this.rightLeg, {loop: false, override: true}).to({rotation: -20}, 250).to({rotation: 0}, 400);
    ...
    this.socket.on('startHold', () => {
      if (this.holdProgress === 4 && this.started === false) {
        ...
        createjs.Tween.get(this.progress, {loop: false, override: true}).to({strokeWidth: 0}, 250, createjs.Ease.easeOut);
        createjs.Tween.get(this.button.scaling, {loop: false, override: true}).to({x: 0, y: 0}, 250, createjs.Ease.easeOut);
        createjs.Tween.get(this.buttonText, {loop: false, override: true}).to({opacity: 0}, 150, createjs.Ease.easeOut);
  ...
  gameSetup() {
    ...
    // this.headLane.pivot = new Point(paper.view.center.x + 92, this.metrumY + 208);
